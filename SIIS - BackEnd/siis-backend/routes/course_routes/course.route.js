...
router.get('/', async (req, res) => {
    ...
    else if(Object.keys(req.query).length === 1 && Object.keys(req.query)[0] === 'lecturer'){
        console.log('Query parameters set:{lecturer}');
        const courses = await Course.find({lecture: req.query.lecturer});
        res.send(courses);
    }
    else{
